"""Per-program benefit calculators for Texas benefit programs."""
...
def calc_tanf(annual_income: float, profile: BenefitsProfile) -> float:
    """TANF: Texas benefits, hard cutoff."""
    hs = min(profile.household_size, 8)
...
def calc_medicaid(annual_income: float, profile: BenefitsProfile) -> float:
    """Medicaid adults: Texas did NOT expand -- not eligible."""
    return 0.0


def calc_chip(annual_income: float, profile: BenefitsProfile) -> float:
    """CHIP (Children's Health Insurance Program): 200% FPL."""
    hs = min(profile.household_size, 8)
    fpl = FPL_2026[hs]
    children = profile.dependents_under_6 + profile.dependents_6_to_17
    if children == 0 or annual_income > fpl * CHIP_FPL_PCT:
        return 0.0
...
def calc_childcare(annual_income: float, profile: BenefitsProfile) -> float:
    ...
    copay_pct = CHILDCARE_COPAY_TIERS[-1][1]
...
def calc_ceap(annual_income: float, profile: BenefitsProfile) -> float:
    """CEAP: 150% FPL, seasonal (~$85/month average)."""
    hs = min(profile.household_size, 8)
    fpl = FPL_2026[hs]
    if annual_income > fpl * CEAP_FPL_LIMIT_PCT:
        return 0.0
    return CEAP_AVG_MONTHLY
