...
@lru_cache(maxsize=8)
def load_employers(city_slug: str) -> tuple[dict, ...]:
    """Load employer data from the city's data directory.

    Returns empty tuple if file is missing or malformed.
    Cached per city_slug to avoid repeated disk reads.
    """
...
def get_fair_chance_employers(city_slug: str) -> list[dict]:
    ...
    return [e for e in load_employers(city_slug) if isinstance(e, dict) and e.get("fair_chance") is True]
