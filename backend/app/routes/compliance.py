...
def download_export(token: str = Query(...)) -> Response:
    ...
    from app.modules.compliance._audit import hash_session_id, write_audit
    write_audit(
    ...
    try:
        ...
        raise
    # hash prevents CRLF/quote injection into Content-Disposition
    safe_id = hash_session_id(session_id)[:16]
    return Response(
        content=archive,
