...
status: pending
sprint: '23'
tags: [assessments, sprint-23, crud]
depends_on: [T23.1]
---
...
- [ ] `record_review(session, version_id, reviewer_id, action, comment) -> int` — inserts review row + transitions version status
...
- [ ] `get_published_version(session, slug) -> dict | None` — latest published version + questions
- [ ] `retire_version(session, version_id) -> None` — admin-only path (no route this sprint)
- [ ] State-machine guards enforced at query layer: only approved → published; rejected terminates; published is immutable
